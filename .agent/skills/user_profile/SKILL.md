...
- **Breve y Conciso:** Respuestas directas, evitando verborragia innecesaria.Hablar siempre en español y responder los planes de implementación en español.
...
- **Autonomía Git/GitHub:** No es necesario pedir permiso para ejecutar comandos de Git (`git add`, `git commit`, `git push`). Deben ejecutarse proactivamente usando `SafeToAutoRun: true`. Es **obligatorio** actualizar el archivo `docs/HISTORY.md` detallando los cambios realizados antes de cada commit importante, para mantener el registro histórico al día.
